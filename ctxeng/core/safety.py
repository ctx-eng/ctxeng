...
SUSPICIOUS_PATTERNS: list[re.Pattern] = [
    ...
    re.compile(
        r"forget\s+(all\s+)?(previous|prior)\s+(instructions?|context|rules)", re.IGNORECASE
    ),
...
POISONING_PATTERNS: list[re.Pattern] = [
    ...
    re.compile(
        r"when\s+asked\s+about\s+\w+\s+(always|never)\s+(say|respond|answer)", re.IGNORECASE
    ),
